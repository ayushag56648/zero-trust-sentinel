...
function TimelineDot({ status }: { status: StageStatus }) {
  ...
  return (
    <div className={`relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border ${ring}`}>
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h2 className="text-lg font-semibold text-foreground">
            Live Threat Analysis &amp; CDR Pipeline
          </h2>
          <p className="text-xs text-muted-foreground mt-0.5">Real-time file processing status</p>
        </div>
        <span className={`flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium ${headerColor}`}>
      ...
      {pipelineStatus === 'idle' && (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">
            Upload a file on the left to begin scanning
          </p>
        </div>
      )}

      {/* Timeline — only shown once a scan starts */}
      {pipelineStatus !== 'idle' && (
        <div className="space-y-0">
          {STAGES.map(({ id, title, subtitle }, idx) => {
            const s      = stages[id]
            const isLast = idx === STAGES.length - 1
            const badge  = id === 'scan' && s.riskLevel ? RISK_BADGE[s.riskLevel] : null
            ...
            return (
              <div key={id} className="flex gap-4">
                ...
                <div className="flex flex-col items-center">
                  ...
                  {!isLast && (
                    <div className={`w-px flex-1 my-1 min-h-[1.5rem]
                      ${s.status === 'done' ? 'bg-primary/30' : 'bg-border/30'}`} />
                  )}
                </div>
                ...
                >
                  ...
                  {/* Status message */}
                  {s.status === 'waiting' && (
                    <p className="font-mono text-xs text-muted-foreground/40">Waiting…</p>
                  )}
                  {(s.status === 'running' || s.status === 'done') && s.message && (
                    <p className={`font-mono text-xs
                      ${s.status === 'running' ? 'text-amber-400' : 'text-primary'}`}>
                      {s.message}
                    </p>
                  )}
                  {s.status === 'error' && (
                    <p className="font-mono text-xs text-destructive">
                      {s.message || 'Stage failed'}
                    </p>
                  )}

                  {/* Threat signals (scan stage only) */}
                    ...
                    <div className="mt-3 space-y-1 max-h-36 overflow-y-auto">
                      {(s.signals as ThreatSignal[]).map((sig, i) => (
                        <div key={i} className="flex items-start gap-1.5 text-xs">
                          <span className={`mt-1 h-1.5 w-1.5 shrink-0 rounded-full ${SEV_DOT[sig.severity] ?? 'bg-muted'}`} />
                          <span className={`font-medium shrink-0 ${SEV_COLOR[sig.severity]}`}>
                            {sig.severity}
                          </span>
      ...
      {pipelineStatus === 'success' && result && (() => {
        ...
        return (
          <div className="mt-2 rounded-lg border border-primary/20 bg-primary/5 px-4 py-3 flex items-center justify-between">
            ...
            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${b.bg} ${b.text}`}>
              {b.label}
            </span>
          </div>
        )
